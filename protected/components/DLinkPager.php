 ...
 * @since 1.0
 */
class DLinkPager extends CBasePager
{
	...
	protected function createPageButtons()
	{
		...
		if ($this->firstPageLabel !== false) {
			$buttons[]=$this->createPageButton($this->firstPageLabel,0,$this->firstPageCssClass,$currentPage<=0,false, -1);
		}
		// prev page
		if ($this->prevPageLabel !== false) {
			if(($page=$currentPage-1)<0)
				$page=0;
			$buttons[]=$this->createPageButton($this->prevPageLabel,$page,$this->previousPageCssClass,$currentPage<=0,false, -1);
		}
		...
		if ($this->nextPageLabel !== false) {
			if(($page=$currentPage+1)>=$pageCount-1)
				$page=$pageCount-1;
			$buttons[]=$this->createPageButton($this->nextPageLabel,$page,$this->nextPageCssClass,$currentPage>=$pageCount-1,false, 1);
		}
		// last page
		if ($this->lastPageLabel !== false) {
			$buttons[]=$this->createPageButton($this->lastPageLabel,$pageCount-1,$this->lastPageCssClass,$currentPage>=$pageCount-1,false, 1);
		}
	 ...
	 */
	protected function createPageButton($label,$page,$class,$hidden,$selected,$number = 0)
	{
		...
		switch ($number) {
			case 1:
				$link = CHtml::link($label,$this->createPageUrl($page), array('rel' => 'next'));
				break;

			case -1:
				$link = CHtml::link($label,$this->createPageUrl($page), array('rel' => 'prev'));
				break;
