...
class DefaultController extends BaseController
{
	...
	public function filters()
	{
		return A::m(parent::filters(), [
			...
			'ajaxOnly + changeStatus,changeCompleted, changePaid, detail, updateComment, delete'
		]);
	}
    ...
    public function actionChangeCompleted()
	{
        ...
        $clientEmail = $customer['E-mail']['value'];

        // $message = $this->renderPartial('//mail/order/change_status', [
        //     ['order' => R::post('item'), 'status' =>R::post('status')],
        //     true
        // ]);
                    HEmail::cmsSend(
                       $clientEmail,
                       'Изменение статуса заказа №'.R::post('item'),
                       ['order' => R::post('item'), 'status' =>R::post('status')],
                       '//mail/order/change_status'
                   );
        //$headers = 'MIME-Version: 1.0' . "\r\n";
        //$headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";

        //mail( R::post('mail'), "ТЕМА ПИСЬМА", "".$message."", "".$headers."");
